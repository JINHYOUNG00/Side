...
**웹은 반응형으로 대응한다**: 모바일 레이아웃(< 900px)은 위 모바일 퍼스트 디자인 그대로 유지(향후 Flutter 이식의 시각 기준이라 보존). 분기점 900px(`tokens.css --bp-wide`, @media는 var를 못 쓰므로 리터럴 동기화) 이상의 데스크톱에서만 CSS 미디어쿼리로 레이아웃을 얹는다 — 하단 탭(BottomNav) → 좌측 사이드바(SideNav), 바텀시트(BottomSheet) → 화면 중앙 모달, 본문은 중앙 정렬 후 폭 제한(`--content-w`). 컴포넌트 동작·문구·토큰은 불변, 레이아웃만 분기(JS matchMedia 분기 금지 — 테스트 안정). 브랜드 로고는 `BrandLogo.vue`(흐르는 물방울 마크 + `app.name` 워드마크, size로 비례 확대). `화면설계.html`은 모바일 시각 정본으로 유지.
