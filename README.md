...
Phase 0~6 기능 구현 완료 + Phase 7 일부 — [feature_list.json](feature_list.json) **64건 중 61건 verify 통과**(단위·통합·ArchUnit·골든 + 프론트 컴포넌트 테스트), 나머지 3건은 미착수 백로그(전부 Phase 7). 진행 이력은 [progress.md](progress.md), 마일스톤은 [개발계획](docs/개발계획.md#6-개발-마일스톤) 참조.

- **구현됨**: OAuth 로그인·온보딩, 통장·항목 CRUD, 월급 폭포(저축률 포함), 월급날 체크리스트·지급일 조정, 비정기 지출 봉투(적립·지출·주기 갱신), 보관함·만기금액 계산, 노션 임포트, 일 단위 입력(ITEM-03), 지급일/지출일/체크인 알림, 점검 리마인더(분기 외화 점검·사용자 정의, NOTI-06), 추이·요약 리포트·월말 체크인, 언어 설정, 보정/리밸런싱 제안(SUG·MOD-06), 여윳돈/부족 인터랙티브 배분(CYCLE-05), 신규 사용자 빈 상태(RPT-03), 회원 탈퇴(AUTH-04, 전 도메인 cascade 물리 삭제)
- **남은 작업**(미착수 백로그, 전부 Phase 7): 네이버 로그인(AUTH-02) · 내보내기(DATA-02) · 연간 리포트(RPT-04) · 라이브 E2E(실 OAuth/SMTP) · 모바일(Flutter)
