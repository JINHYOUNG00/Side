...
Phase 0~7 기능 구현 완료 — [feature_list.json](feature_list.json) **64건 전건 verify 통과**(단위·통합·ArchUnit·골든 + 프론트 컴포넌트 테스트). 네이버 로그인(AUTH-02)은 코드 배선 완료·verify 통과 상태이며, 실제 라이브 동작은 네이버 외부 검수 승인·실 credentials 주입이 전제다. 진행 이력은 [progress.md](progress.md), 마일스톤은 [개발계획](docs/개발계획.md#6-개발-마일스톤) 참조.

- **구현됨**: OAuth 로그인(카카오·구글·네이버 배선, AUTH-02)·온보딩, 통장·항목 CRUD, 월급 폭포(저축률 포함), 월급날 체크리스트·지급일 조정, 비정기 지출 봉투(적립·지출·주기 갱신), 보관함·만기금액 계산, 노션 임포트·내보내기(DATA-02, 마크다운/CSV·임포트 라운드트립), 일 단위 입력(ITEM-03), 지급일/지출일/체크인 알림, 점검 리마인더(분기 외화 점검·사용자 정의, NOTI-06), 추이·요약 리포트·월말 체크인·연간 결산(RPT-04, 연 단위 저축률·만기 수령·봉투 집행), 언어 설정, 보정/리밸런싱 제안(SUG·MOD-06), 여윳돈/부족 인터랙티브 배분(CYCLE-05), 신규 사용자 빈 상태(RPT-03), 회원 탈퇴(AUTH-04, 전 도메인 cascade 물리 삭제)
- **남은 작업**(트래커 밖 라이브 영역): 라이브 E2E(실 OAuth/SMTP) · 네이버 외부 검수 신청·실 credentials 주입 · 운영 배포 · 모바일(Flutter)
