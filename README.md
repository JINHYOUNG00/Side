...
Phase 0~5 핵심 기능 구현 완료 — [feature_list.json](feature_list.json) **52건 전부 verify 통과**(단위·통합·ArchUnit·골든 + 프론트 컴포넌트 테스트). 진행 이력은 [progress.md](progress.md), 마일스톤은 [개발계획](docs/개발계획.md#6-개발-마일스톤) 참조.

- **구현됨**: OAuth 로그인·온보딩, 통장·항목 CRUD, 월급 폭포(저축률 포함), 월급날 체크리스트·지급일 조정, 비정기 지출 봉투(적립·지출·주기 갱신), 보관함·만기금액 계산, 노션 임포트, 지급일/지출일/체크인 알림, 추이·요약 리포트·월말 체크인, 언어 설정
- **남은 작업**: 보정 제안·리밸런싱(SUG, Phase 6) · 연간 리포트(RPT-04) · 사이클 스냅샷 운영 트리거(봉투 적립 라인 emit) · 라이브 E2E(실 OAuth/SMTP) · 모바일(Flutter)
